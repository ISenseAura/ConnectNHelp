...
function importTestmons() {
  let doc = document.getElementById("testmons");
  const dbRefObject =  firebase.database().ref("testmons");
  dbRefObject.on('value', snap => {
  let json = {}
    json.data = snap.val();
    json.success = true;
    if(json.success) {
      dt = json.data;
  dt.forEach((a) => {
    let ht = `    <div class="col-sm-6 border-end border-bottom p-4">
    <div class="row">
        <div class="col-md-3">
            <img src="${a.image}" class="img-thumbnail rounded-circle" alt="User">
        </div>
        <div class="col-md-9">
            <figure>
                <blockquote class="blockquote mt-3">
                    <p class="fs-6">${a.testimonial}</p>
                </blockquote>
                <figcaption class="blockquote-footer">${a.name}</figcaption>
            </figure>
        </div>
    </div>
</div>
